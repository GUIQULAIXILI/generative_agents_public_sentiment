...
sys.path.append('../../')
# from reverie.global_methods import *
from global_methods import *
# from reverie.backend_server.persona.prompt_template.gpt_structure import *
# from reverie.backend_server.persona.prompt_template.print_prompt import *
...
def run_gpt_prompt_rumor_poignancy(persona, event_description, test_input=None, verbose=False):
    def create_prompt_input(persona, event_description, test_input=None):
        prompt_input = [persona.scratch.name,
                        persona.scratch.get_str_iss(),
                        persona.scratch.name,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "persona/prompt_template/v3_ChatGPT/poignancy_rumor_v1.txt"  ########
    prompt_input = create_prompt_input(persona, event_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "15"  ########
    special_instruction = "The output should ONLY contain ONE integer value on the scale of 10 to 20."  ########
    fail_safe = get_fail_safe()  ########
    output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                            __chat_func_validate, __chat_func_clean_up, True)
    if output != False:
...
def run_gpt_prompt_on_init_personinfo(comment, test_input=None, verbose=False):
    def create_prompt_input(comment, test_input=None):
        prompt_input = [comment['content'], comment['sex']]
        return prompt_input
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 250,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "D:/Generative Agents/generative_agents-main/reverie/backend_server\persona\prompt_template/v3_ChatGPT/init_personinfo_v1.txt"  ########
    prompt_input = create_prompt_input(comment)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "name: Maria Lopez$first_name: Maria$last_name: Lopez$age: 21$innate: energetic, enthusiastic, inquisitive$daily_plan: Maria Lopez spends at least 3 hours a day Twitch streaming or gaming$learned: Maria Lopez is a student at Oak Hill College studying physics and a part time Twitch game streamer who loves to connect with people and explore new ideas.$currently: Maria Lopez is working on her physics degree and streaming games on Twitch to make some extra money. She visits Hobbs Cafe for studying and eating just about everyday.$lifestyle: Maria Lopez goes to bed around 2am, awakes up around 9am, eats dinner around 6pm. She likes to hang out at Hobbs Cafe if it's before 6pm."  ########
    special_instruction = ''
    fail_safe = get_fail_safe()  ########
    output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                            __chat_func_validate, __chat_func_clean_up, True)
...
def run_gpt_prompt_convo_is_about_rumor(convo, convo_summary, test_input=None):
    def create_prompt_input(conversation, conversation_summary ,test_input=None):
        convo_str = ""
        for row in conversation:
            convo_str += f'{row[0]}: "{row[1]}"\n'
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "persona/prompt_template/v3_ChatGPT/convo_is_about_rumor_v1.txt"  ########
    prompt_input = create_prompt_input(convo, convo_summary, test_input)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "2"  ########
    special_instruction = "The output must be the number 1 or 2. When the output is 1, it means that the conversation content does not involve social public opinion and does not reflect one's position. When the output is 2, it means that the conversation content involves social public opinion or is to express one's own opinions, thoughts or attitudes on certain news, certain special events, certain rumors or social public events."  ########
    fail_safe = get_fail_safe()  ########
    output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                            __chat_func_validate, __chat_func_clean_up, True)
    if output != False:
