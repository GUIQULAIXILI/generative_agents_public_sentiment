  ...
  for p_event in perceived_events:
    ...
    if p_event not in latest_events:
      ...
      if p_event[0] == f"{persona.name}" and p_event[1] == "chat with": 
        ...
        ids = chat_node_ids
      elif p_event[0] == f"{persona.name}" and p_event[1] == "talk about a rumor with":
        curr_event = persona.scratch.act_event
        if persona.scratch.act_description in persona.a_mem.embeddings:
          rumor_embedding = persona.a_mem.embeddings[
                             persona.scratch.act_description]
        else:
          rumor_embedding = get_embedding(persona.scratch
                                                .act_description)
        rumor_embedding_pair = (persona.scratch.act_description,
                               rumor_embedding)
        rumor_poignancy = generate_poig_score(persona, "rumor",
                                             persona.scratch.act_description)
        rumor_node = persona.a_mem.add_rumor(persona.scratch.curr_time, None,
                      curr_event[0], curr_event[1], curr_event[2],
                      persona.scratch.act_description, keywords,
                      rumor_poignancy, rumor_embedding_pair,
                      persona.scratch.chat)
        rumor_node_ids = [rumor_node.node_id]
        ids = rumor_node_ids

      # Finally, we add the current event to the agent's memory.
      ret_events += [persona.a_mem.add_event(persona.scratch.curr_time, None,
                           s, p, o, desc, keywords, event_poignancy,
                           event_embedding_pair, ids)]
