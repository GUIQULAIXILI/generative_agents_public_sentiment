...
def convo_is_about_rumor(convo, convo_summary):
  if debug: print ("GNS FUNCTION: <convo_is_about_rumor>")
  if int(run_gpt_prompt_convo_is_about_rumor(convo, convo_summary)[0]) == 1:
    return False
  elif int(run_gpt_prompt_convo_is_about_rumor(convo, convo_summary)[0]) == 2:
    return True
  else:
    return False
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas):
  ...
  if convo_is_about_rumor(convo, convo_summary):
    for role, p in [("init", init_persona), ("target", target_persona)]:
      if role == "init":
        act_address = f"<persona> {target_persona.name}"
        act_event = (p.name, "talk about a rumor with", target_persona.name)
        chatting_with = target_persona.name
        chatting_with_buffer = {}
        chatting_with_buffer[target_persona.name] = 800
      elif role == "target":
        act_address = f"<persona> {init_persona.name}"
        act_event = (p.name, "talk about a rumor with", init_persona.name)
...
def plan(persona, maze, personas, new_day, retrieved): 
  ...
  if persona.scratch.act_event[1] != "chat with" and persona.scratch.act_event[1] != "talk about a rumor with":
