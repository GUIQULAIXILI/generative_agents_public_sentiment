...
def generate_poig_score(persona, event_type, description): 
  ...
  if event_type == "event" or event_type == "thought": 
    ...
    return run_gpt_prompt_rumor_poignancy(persona,
                                         persona.scratch.act_description)[0]
...
def generate_planning_thought_on_convo_rumor(thought):
  if debug: print ("GNS FUNCTION: <generate_planning_thought_on_convo>")
  return Expert_system_rumor_request(thought)
  ...
  if persona.scratch.chatting_end_time: 
    ...
    if persona.scratch.curr_time + datetime.timedelta(0,10) == persona.scratch.chatting_end_time: 
     ...
     #plan_thought
      ...
      planning_thought = generate_planning_thought_on_convo(persona, all_utt)
      planning_thought_rumor= generate_planning_thought_on_convo_rumor(planning_thought)
      planning_thought_rumor = f"For {persona.scratch.name}'s planning,the next time {persona.scratch.name} meets someone else, {persona.scratch.name} tends to relay the following content from {persona.scratch.name}'s perspective to others: {planning_thought_rumor}"
      created = persona.scratch.curr_time
      expiration = persona.scratch.curr_time + datetime.timedelta(days=30)
      s, p, o = generate_action_event_triple(planning_thought_rumor, persona)
      keywords = set([s, p, o])
      thought_poignancy = generate_poig_score(persona, "thought", planning_thought_rumor)
      thought_embedding_pair = (planning_thought_rumor, get_embedding(planning_thought_rumor))

      evidence = [persona.a_mem.get_last_chat(persona.scratch.chatting_with).node_id]

      persona.a_mem.add_thought(created, expiration, s, p, o,
                                planning_thought_rumor, keywords, thought_poignancy,
                                thought_embedding_pair, evidence)
